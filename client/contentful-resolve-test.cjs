...
const url = `https://cdn.contentful.com/spaces/${spaceId}/environments/${environment}/entries?content_type=blogPost&order=-sys.publishedAt&limit=1&include=2&access_token=${accessToken}`;
...
const buildIncludeMap = (includes) => {
  ...
  assets.forEach((asset) => {
    if (asset?.sys?.id) map[asset.sys.id] = asset;
  });
  return map;
};
...
(async () => {
  ...
  const imageUrl = resolveAssetUrl(item?.fields?.image, includeMap);
  console.log('imageUrl:', imageUrl);
  console.log('raw image link:', JSON.stringify(item?.fields?.image, null, 2));
  console.log('includeMap keys:', Object.keys(includeMap));
})();
